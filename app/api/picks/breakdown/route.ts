...
export async function GET() {
  ...
  const [matchPicks, bracketPicks, matchResults, bracketResults] = await Promise.all([
    ...
    prisma.bracketResult.findMany().catch(() => [] as { round: string; slot: number; team: string }[]),
  ]);

  const resultMap = new Map(matchResults.map((r) => [r.matchId, r.result]));
  const bracketMap = new Map(bracketResults.map((r: { round: string; slot: number; team: string }) => [`${r.round}-${r.slot}`, r.team]));
  ...
  const total = groupCorrect * 1 + groupWrong * (-1) + bracketTotal;
